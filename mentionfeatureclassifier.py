...
mention_categories = ['PER.fic','PER.nfic','PER.misc','LOC.heelal',
                      'LOC.water','LOC.bc','LOC.land','LOC.regio','LOC.lijn',
                      'LOC.punt','LOC.fic','ORG.gov','ORG.com','ORG.misc',
                      'PKM.in','PKM.uit','OBJ','MISC']
...
def evaluate(validationfiles, parsesdir, annotations, tokenizer, bertmodel):
	...
	print(classification_report(
			np.array(y_val, dtype=bool),
			np.array([prob2labels(a) for a in probs], dtype=bool),
			target_names=target_names,
			zero_division=0,
			digits=3))


def prob2labels(probs):
	labels = np.zeros(len(probs), dtype=bool)
	labels[:7] = probs[:7] > 0.5
	labels[7 + probs[7:].argmax()] = True
	return labels
